public class MatrixArrayCR {
	...
	public MatrixArrayCR(long [] input){//this method assumes n x n matrix
		int counter=0;
		...
		for (int i =0; i<=size-1; i++){//each row
			for (int j=0; j<=size-1; j++){//each column
				this.mValues[j][i]=input[counter];
				counter++;
			}
		}
	}
	public long determinant(){
		return calculate(mValues);//transpose matrix determinant is same as normal matrix determinant
	}
